...
当用户询问省份、城市、区县时必须通过地址信息和工具get_address_info获取,不能直接从详细地址、名称或者大模型内部信息中获取,必须通过给出的工具查询。
...
回答时严格按照工具返回信息进行回答，比如询问案件审理时间是什么时候，返回信息是{{"日期": "2021-06-16 00:00:00"}},必须回答：案件审理时间是2021-06-16 00:00:00。
...
def process_query(all_tools, query, update_message=True, suggested_logic_chain=False):
    ...
    if route == "诉讼书":
        ...
        result = run_report(query["question"], None, None)
    else:
...
def process_query_and_put_in_queue(all_tools, query, order_queue, update_message=True):
    try:
        content = process_query(all_tools, query, update_message=update_message)
        order_queue.put((query["id"], content))
    except Exception as e:
        content = {"id": query["id"], "question": query["question"], "answer": ""}
        order_queue.put((query["id"], content))
        print(f"Error processing query {query['id']}: {e}")


# 使用线程池来处理queries
def process_queries_in_parallel(all_tools, queries, update_message=True):
    order_queue = Queue()
    with ThreadPoolExecutor(max_workers=4) as executor:
        # 提交任务到线程池
        futures = [
            executor.submit(process_query_and_put_in_queue, all_tools, query, order_queue, update_message)
...
if __name__ == "__main__":
    if True:
        ...
        # 修改输出文件
        result_file = "result_84_1.json"
        # result_file = "result_83_1_fix1.json"
        ...
        for query in tqdm(queries):
            ...
            if query["id"] < 163:
                continue
            # content = process_query(all_tools, query)
            # content = process_query(all_tools, query, False, True)
            content = process_query(all_tools, query, False, False)
        ...
        result_file = "result_82_166_0520_fixed.json"
        ...
        for incorrect_answer in incorrect_answers:
            print(str(incorrect_answer["id"]) + ": " + incorrect_answer["question"])
        ...
        print_log(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
        results_in_order = process_queries_in_parallel(all_tools, queries)
        with jsonlines.open(result_file, "a") as json_file:
